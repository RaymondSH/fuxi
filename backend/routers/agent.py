...
def start_run(req: RunRequest, request: Request,
    ...
    with pool.connection() as conn:
        ...
        conn.execute(
            ...
            "'queued','queued','治理 Agent 规划')", (job_id, run_id, req.space_id)
        )
...
def _review(proposal_id, approve, user, request):
    with pool.connection() as conn:
        row = conn.execute("SELECT space_id,status FROM agent_proposals WHERE id=%s",
                           (proposal_id,)).fetchone()
    if row is None: raise HTTPException(status_code=404, detail="提案不存在")
    spaces.assert_space_role(user, row[0], "space_admin")
    if row[1] != "pending": raise HTTPException(status_code=409, detail="提案已处理")
    status = "approved" if approve else "rejected"
    with pool.connection() as conn:
        conn.execute("UPDATE agent_proposals SET status=%s,reviewed_by=%s,reviewed_at=NOW() WHERE id=%s",
