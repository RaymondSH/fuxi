...
镜像 ingest_worker 的两段式入口：
  ...
  run               后台跑编译流程，供 BackgroundTasks 调用
...
def compile_sync(slug: str, title: str, source_note_ids: list[uuid.UUID]) -> str:
    """同步跑完（脚本 / 迁移用），返回 slug。"""
    enqueue_compile(slug, title, source_note_ids)
    run(slug)
    return slug


def enqueue_compile(slug: str, title: str, source_note_ids: list[uuid.UUID]) -> str:
    """建/更新 wiki_pages 占位行 + 一条 compile job，返回 slug。处理交给 run()。"""
    with pool.connection() as conn:
        conn.execute(
            """
            INSERT INTO wiki_pages (slug, title, source_note_ids)
            VALUES (%s, %s, %s)
        ...
        conn.execute(
            ...
            (json.dumps({"slug": slug, "title": title}),),
        )
    return slug


def run(slug: str) -> None:
    """后台执行编译流程。供 BackgroundTasks 调用。"""
    _process(slug)


# ---------- 主流程 ----------

def _process(slug: str) -> None:
    try:
...
def _fetch_sources(slug: str) -> list[dict]:
    ...
    with pool.connection() as conn:
        row = conn.execute(
            "SELECT source_note_ids FROM wiki_pages WHERE slug = %s", (slug,)
        ).fetchone()
        ...
        rows = conn.execute(
            ...
            FROM notes WHERE id = ANY(%s) AND ingest_status = 'done'
            """,
            (ids,),
        ).fetchall()
